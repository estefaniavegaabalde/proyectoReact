...
const Cart = () => {
    ...
    if (totalQuantity === 0) {
        return (
            <div className="empty-cart-container my-4 text-center">
                <h1 className="h5 mb-2">No hay productos en el carrito</h1>
                <Link to="/" className="btn" style={{ backgroundColor: "rgb(106, 90, 205)", color: "white" }}>
                    Ir al inicio
                </Link>
            </div>
        );
        ...
        return (
            <div>
                {cart.map((product) => (
                    <CartItem key={product.id} {...product} onRemove={() => handleRemoveItem(product.id)} />
                ))}
                ...
                <div className="d-flex justify-content-center mt-3">
                    <button
                        onClick={() => clearCart()}
                        className="btn"
                        style={{ backgroundColor: "rgb(106, 90, 205)", color: "white", marginRight: "10px" }}
                    >
                        Limpiar carrito
                    </button>
                    <Link
